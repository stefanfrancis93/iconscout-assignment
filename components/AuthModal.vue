<template>
  <Teleport to="body">
    <Transition name="modal-slide-fade">
      <div
        v-if="show"
        class="fixed inset-0 z-[1040] flex items-center justify-center"
      >
        <div class="fixed inset-0 bg-black/70" @click="close" />
        <div
          class="relative z-10 bg-white rounded-2xl md:rounded-4xl shadow-xl w-full max-w-[870px] mx-auto my-8"
        >
          <UButton
            class="absolute top-4 right-4 p-2 cursor-pointer bg-transparent hover:bg-white-100 rounded-full"
            @click="close"
          >
            <img
              src="https://cdna.iconscout.com/img/close.86b735b.svg"
              height="24"
              alt="Close"
            />
          </UButton>
          <div class="flex flex-col md:flex-row items-stretch p-4">
            ...
            >
              <img
                src="https://cdna.iconscout.com/img/auth-visual-1.71cea32.png"
                alt="Features"
                class="w-[450px] max-w-[80%]"
              />
            </div>
            <div class="flex flex-1 flex-col justify-center p-6 max-w-[50%]">
              <h4 class="text-4xl font-bold mb-6 text-black text-center">
              ...
              <div v-if="mode === 'login'" class="text-center mt-5 pb-8">
                <a
                  href="#"
                  class="text-gray-250 text-sm hover:text-gray-450 hover:underline"
                  >Forgot Password?</a
                >
              </div>
              <div class="text-center pt-8 text-sm border-t-1 border-white-100">
                ...
                <button
                  class="ml-1 text-brand-darker font-base hover:underline cursor-pointer"
                  @click="toggleMode"
                >
